...
public class BlogPostRepository : GenericRepository<BlogPost>
{
    ...
    public async Task<IList<BlogPost>> GetFeaturedAsync(int count) =>
        await _dbSet.AsNoTracking()
    ...
    public async Task<IList<BlogPost>> GetRecentAsync(int count) =>
        await _dbSet.AsNoTracking()
    ...
    public async Task<(IList<BlogPost> Posts, int TotalCount)> GetPagedAsync(int page, int pageSize, string? categorySlug = null, string? tagSlug = null, string? search = null)
    {
